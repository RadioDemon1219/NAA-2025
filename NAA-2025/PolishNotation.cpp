...
namespace Polish
{
	int  getPriority(LT::Entry& e)
	{
		...
		case LEX_LEFTSHIFT: case LEX_RIGHTSHIFT: return 3; // заменено LEX_LEFT/LEX_RIGHT -> LEX_LEFTSHIFT/RIGHTSHIFT
		case LEX_EXP: return 3;
		default: return -1;
		}
	}
	...
	{
		intvec v;
		bool f_begin = false; // признак найденного начала выражения
		bool f_end = false;  // признак найденного конца выражения
		...
		for (int i = 0; i < tbls.lextable.size; i++)
		{
			...
			if (f_begin && f_end)	// добавить начало выражения в вектор
			{
				v.push_back(begin);
				f_begin = f_end = false;
			}
	...
	bool __cdecl setPolishNotation(IT::IdTable& idtable, Log::LOG& log, int lextable_pos, ltvec& v)
	{
		...
		for (unsigned i = 0; i < v.size(); i++)
		{
			...
			if (v[i].lexema == LEX_LEFTHESIS || v[i].lexema == LEX_RIGHTTHESIS || v[i].lexema == LEX_PLUS || v[i].lexema == LEX_MINUS || v[i].lexema == LEX_STAR || v[i].lexema == LEX_DIRSLASH || v[i].lexema == LEX_LEFTSHIFT || v[i].lexema == LEX_RIGHTSHIFT || v[i].lexema == LEX_EXP)
			{
			...
			// заменено побитовое & на логическое &&, добавлены все допустимые лексемы
			if (v[i].lexema != LEX_LEFTHESIS && v[i].lexema != LEX_RIGHTTHESIS && v[i].lexema != LEX_PLUS && v[i].lexema != LEX_MINUS && v[i].lexema != LEX_STAR && v[i].lexema != LEX_DIRSLASH && v[i].lexema != LEX_ID && v[i].lexema != LEX_LITERAL && v[i].lexema != LEX_LEFTSHIFT && v[i].lexema != LEX_RIGHTSHIFT && v[i].lexema != LEX_EXP)
			{
